...
void Domain::collectProfile(DomainDecompBase* dode)
{
	...
	for(unsigned unID = 0; unID < unIDs; unID++)
	{
		...
		for(unsigned short d=0; d<3; d++){
			...
			dode->collCommAppendLongDouble(_localV2Profile[d][unID]);
		}
...
void Domain::resetProfile()
{
	...
	for(unsigned unID = 0; unID < unIDs; unID++)
	{
		...
		for(int d=0; d<3; d++)
		{
			...
			this->_globalV2Profile[d][unID] = 0.0;
		}
...
void Domain::setupSlabProfile(unsigned xun, unsigned yun, double widthZ){
  this->_universalNProfileUnits[0] = xun;
  this->_universalNProfileUnits[1] = yun;
  ...
  _universalInvProfileUnit[0] = _universalNProfileUnits[0] / _globalLength[0];
  _universalInvProfileUnit[1] = _universalNProfileUnits[1] / _globalLength[1];
  _universalInvProfileUnit[2] = 1.0/widthZ;  
}
  ...
  double mv2, Iw2;
	for(Molecule* thismol = molCont->begin(); thismol != molCont->end(); thismol = molCont->next())
	{
		...
		if(this->_universalProfiledComponents[cid])
		{
			...
			unID = xun * this->_universalNProfileUnits[1]  + yun;  
			}
			else{
				continue;
			}
			...
			for(int d=0; d<3; d++){
			  ...
			  this->_localV2Profile[d][unID]+= (thismol->v(d)*thismol->v(d));
			}
...
void Domain::outputSlabProfile(const char* prefix){
	...
	rhoProf << "// T \t sigma_ii \t eps_ii \t DELTA_x \t DELTA_y \t DELTA_z, i.e. slab width\n";
	...
	for(unsigned n_y = 0; n_y < this->_universalNProfileUnits[1]; n_y++)
	{
	  double yval = (n_y + 0.5) / this->_universalInvProfileUnit[1];
	  rhoProf << yval<< "  \t";
	  for(unsigned n_x = 0; n_x< this->_universalNProfileUnits[0]; n_x++)
	  {
	    unsigned unID = n_x * this->_universalNProfileUnits[1]  + n_y;
...
//! can be used for any kind of profile, i.e. slab profile too
	   ...
	   for( map<unsigned, bool>::iterator pcit = _universalProfiledComponents.begin();    // Loop ueber alle Komponenten
	    ...
	    //##########################################################################################################################################
			 ...
			 *tmpProf << "//one single matrix of the local temperature T(r_i,h_i) \n//      | r_i\n//---------------------\n//  h_i| T(r_i,h_i)\n//      | \n";
			 ...
			 //##########################################################################################################################################			 
			 // y-velocity absolute value profile: actual writing procedure 
	   	        	...
	   	        	for(unsigned n_h = 0; n_h < this->_universalNProfileUnits[2]; n_h++)
	   	        	{
	   	        		...
	   	        		*yVelProf << hval<< "  \t";
	   	        		for(unsigned n_r2 = 0; n_r2< this->_universalNProfileUnits[1]; n_r2++)
	   	        		{
	   	        			unsigned unID = n_phi * IDweight[0] + n_r2 * IDweight[1]
	   	        				                                    + n_h * IDweight[2];
						// _globalV2Profile[i] ist the squared velocity component in direction i
	   	        		   	double normVy = sqrt(this->_globalV2Profile[1][unID]);
			 ...
			 //##########################################################################################################################################			 
			 // xz-velocity absolute value profile: actual writing procedure 
	   	        		...
	   	        		*xzVelProf << hval<< "  \t";
	   	        		for(unsigned n_r2 = 0; n_r2< this->_universalNProfileUnits[1]; n_r2++)
	   	        		{
	   	        			unsigned unID = n_phi * IDweight[0] + n_r2 * IDweight[1]
	   	        				                                    + n_h * IDweight[2];
						// _globalV2Profile[i] ist the squared velocity component in direction i
	   	        		   	double normVxz = sqrt(this->_globalV2Profile[0][unID] + this->_globalV2Profile[2][unID]);
