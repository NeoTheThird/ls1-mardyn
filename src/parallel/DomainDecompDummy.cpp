...
void parallel::DomainDecompDummy::collectCosetVelocity(
...
{
   for(map<unsigned, unsigned long>::iterator lNit = localN->begin(); lNit != localN->end(); lNit++)
      (*globalN)[lNit->first] = lNit->second;
   for( map<unsigned, long double>::iterator lVSit = localVelocitySum[0].begin();
        lVSit != localVelocitySum[0].end();
        lVSit ++ )
      for(unsigned d = 0; d < 3; d++)
         globalVelocitySum[d][lVSit->first] = localVelocitySum[d][lVSit->first];
}

void parallel::DomainDecompDummy::broadcastCosetAcceleration(
   map<unsigned, double>* universalAdditionalAcceleration )
{
   // NIL
}

void parallel::DomainDecompDummy::broadcastVelocitySum(
