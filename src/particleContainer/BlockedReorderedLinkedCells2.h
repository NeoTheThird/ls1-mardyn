...
public:
	 ...
	 */
	void linearize(MoleculeArray& molecules, std::vector<int>& cellStartIndices);
	 ...
	 */
	void delinearize(MoleculeArray& molecules);
...
private:
	...
	//! iterator to traverse the list of cells
	std::vector<BlockedCell>::iterator _cellIter;
	unsigned int _cellIteratorIndex;

	//! Iterator to traverse the particles of a cell
	MoleculeArray::iterator _molIter;
	unsigned int _moleculeIteratorIndex;

	std::vector<BlockedCell> _cells; //!< Vector containing all cells (including halo)
...
#endif /*BLOCKEDREORDEREDLINKEDCELLS_H_*/
