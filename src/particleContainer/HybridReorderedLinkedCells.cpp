...
void HybridReorderedLinkedCells::update() {

	std::vector<HybridCell>::iterator celliter;
...
void HybridReorderedLinkedCells::addParticle(Molecule& particle) {
...
void HybridReorderedLinkedCells::traversePairs(ParticlePairsHandler* particlePairsHandler) {
...
unsigned long HybridReorderedLinkedCells::getNumberOfParticles() {
	return _particles.size();
}

Molecule* HybridReorderedLinkedCells::begin() {
...
Molecule* HybridReorderedLinkedCells::next() {
	_particleIter++;
	if (_particleIter != _particles.end()) {
		return &(*_particleIter);
	}
...
Molecule* HybridReorderedLinkedCells::end() {
	return NULL;
}



void HybridReorderedLinkedCells::deleteOuterParticles() {
	if (_cellsValid == false) {
		global_log->error() << "Cell structure in HybridReorderedLinkedCells (deleteOuterParticles) invalid, call update first" << endl;
		exit(1);
	}
	...
	vector<unsigned long>::iterator cellIndexIter;
		...
		for (cellIndexIter = _haloCellIndices.begin(); cellIndexIter != _haloCellIndices.end(); cellIndexIter++) {
			HybridCell& currentCell = _cells[*cellIndexIter];
...
double HybridReorderedLinkedCells::get_halo_L(int index) const {
	return _haloLength[index];
}


void HybridReorderedLinkedCells::getHaloParticles(list<Molecule*> &haloParticlePtrs) {
#ifndef NDEBUG
...
void HybridReorderedLinkedCells::getRegion(double lowCorner[3], double highCorner[3], list<Molecule*> &particlePtrs) {
	if (_cellsValid == false) {
...
void HybridReorderedLinkedCells::getRegion(double lowCorner[3], double highCorner[3], utils::DynamicArray<BasicMolecule, true, false>& particleArray) {
	global_log->error() << "BlockedReorderedLinkedCells::getRegion(DynamicArray<BasicMolecule>&) not implemented!" << endl;
}
...
void HybridReorderedLinkedCells::initializeCells() {
...
void HybridReorderedLinkedCells::calculateNeighbourIndices() {
...
unsigned long HybridReorderedLinkedCells::cellIndexOf3DIndex(int xIndex, int yIndex, int zIndex) const {
	return (zIndex * _cellsPerDimension[1] + yIndex) * _cellsPerDimension[0] + xIndex;
}


void HybridReorderedLinkedCells::deleteMolecule(unsigned long molid, double x, double y, double z) {
...
int HybridReorderedLinkedCells::grandcanonicalBalance(DomainDecompBase* comm) {
	comm->collCommInit(1);
...
Molecule* HybridReorderedLinkedCells::deleteCurrent() {
#ifndef NDEBUG
...
double HybridReorderedLinkedCells::getEnergy(Molecule* m1) {
#ifndef NDEBUG
 ...
 */
unsigned HybridReorderedLinkedCells::countParticles(unsigned int cid) {
 ...
 */
unsigned HybridReorderedLinkedCells::countParticles(unsigned int cid, double* cbottom, double* ctop) {
	#ifndef NDEBUG
		global_log->error() << "TODO: method countParticles(cid) is not implemented / refactored right!" << endl;
		exit(-1);
	#endif
...
void HybridReorderedLinkedCells::grandcanonicalStep(ChemicalPotential* mu, double T) {
#ifndef NDEBUG
