			...
			_blockTraverse(this, _cells, _innerCellIndices, _boundaryCellIndices, _haloCellIndices )
{
	...
	if (_boxWidthInNumCells[0] < _haloWidthInNumCells[0] ||
		...
		global_log->error() << "ReorderedLinkedCells (constructor): bounding box too small for calculated cell length" << endl;
...
void ReorderedLinkedCells::rebuild(double bBoxMin[3], double bBoxMax[3]) {
	for (int i = 0; i < 3; i++) {
	...
	for (int dim = 0; dim < 3; dim++) {
		...
		if (_cellsPerDimension[dim] == 2 * _haloWidthInNumCells[dim]) {
			global_log->error() << "ReorderedLinkedCells::rebuild: region to small" << endl;
			exit(1);
		}
	...
	if (_cellsPerDimension[0] < 3*_haloWidthInNumCells[0] ||
		...
		global_log->error() << "ReorderedLinkedCells (rebuild): bounding box too small for calculated cell Length" << endl;
...
void ReorderedLinkedCells::update() {
	// clear all Cells
	/* XXX for (int i = 0; i < _cells.size(); i++) {
		_cells[i].removeAllParticles();
	} */
...
void ReorderedLinkedCells::addParticle(Molecule& particle) {
...
Molecule* ReorderedLinkedCells::begin() {
...
Molecule* ReorderedLinkedCells::next() {
	_particleIter++;
	if (_particleIter != _particles.end()) {
		return &(*_particleIter);
	}
...
Molecule* ReorderedLinkedCells::end() {
	return NULL;
}

Molecule* ReorderedLinkedCells::deleteCurrent() {
...
void ReorderedLinkedCells::deleteOuterParticles() {
	if (_cellsValid == false) {
		global_log->error() << "Cell structure in ReorderedLinkedCells (deleteOuterParticles) invalid, call update first" << endl;
		exit(1);
	}
...
double ReorderedLinkedCells::get_halo_L(int index) const {
	return _haloLength[index];
}


void ReorderedLinkedCells::getHaloParticles(list<Molecule*> &haloParticlePtrs) {
	if (_cellsValid == false) {
		global_log->error() << "Cell structure in ReorderedLinkedCells (getHaloParticles) invalid, call update first" << endl;
		exit(1);
	}
...
void ReorderedLinkedCells::getRegion(double lowCorner[3], double highCorner[3], list<Molecule*> &particlePtrs) {
	if (_cellsValid == false) {
		global_log->error() << "Cell structure in ReorderedLinkedCells (getRegion) invalid, call update first" << endl;
		exit(1);
	}
	...
	int stopIndex[3];
	int globalCellIndex;
	/* XXX */ std::vector<Molecule*>::iterator particleIter;
	...
	for (int iz = startIndex[2]; iz <= stopIndex[2]; iz++) {
		for (int iy = startIndex[1]; iy <= stopIndex[1]; iy++) {
			for (int ix = startIndex[0]; ix <= stopIndex[0]; ix++) {
				...
				/* XXX std::vector<Molecule*>& particlePointers = _cells[globalCellIndex].getParticlePointers();
				for (size_t i = 0; i < particlePointers.size(); i++) {
					if (particlePointers[i]->r(0) >= lowCorner[0] && particlePointers[i]->r(0) < highCorner[0] &&
						particlePointers[i]->r(1) >= lowCorner[1] && particlePointers[i]->r(1) < highCorner[1] &&
						particlePointers[i]->r(2) >= lowCorner[2] && particlePointers[i]->r(2) < highCorner[2]) {
						particlePtrs.push_back(particlePointers[i]);
					}
				}*/
...
void ReorderedLinkedCells::initializeCells() {
...
void ReorderedLinkedCells::calculateNeighbourIndices() {
...
unsigned long ReorderedLinkedCells::getCellIndexOfMolecule(Molecule* molecule) const {
...
unsigned long ReorderedLinkedCells::cellIndexOf3DIndex(int xIndex, int yIndex, int zIndex) const {
	return (zIndex * _cellsPerDimension[1] + yIndex) * _cellsPerDimension[0] + xIndex;
}


void ReorderedLinkedCells::deleteMolecule(unsigned long molid, double x, double y, double z) {
...
double ReorderedLinkedCells::getEnergy(Molecule* m1) {
	...
	for (molIter2 = currentCell.getParticlePointers().begin(); molIter2 != currentCell.getParticlePointers().end(); molIter2++) {
		...
		if (dd > cutoffRadiusSquare)
			continue;
		u += this->_particlePairsHandler->processPair(*reinterpret_cast<HandlerMoleculeType*>(m1), *reinterpret_cast<HandlerMoleculeType*>(*molIter2), distanceVector, MOLECULE_MOLECULE_FLUID, dd, (dd < LJCutoffRadiusSquare));
	}
...
int ReorderedLinkedCells::grandcanonicalBalance(DomainDecompBase* comm) {
	comm->collCommInit(1);
...
void ReorderedLinkedCells::grandcanonicalStep(ChemicalPotential* mu, double T) {
	bool accept = true;
	double DeltaUpot;
	Molecule* m;
